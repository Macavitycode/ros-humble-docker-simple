...
RUN apt install -y neofetch python3-pip clang wget ca-certificates gnupg \
    ...
    python3-rospkg software-properties-common
...
# Installing Nvim
# RUN add-apt-repository ppa:neovim-ppa/stable && sudo apt update && \
#     apt install neovim

# Installing ros and gazebo packages
RUN apt update && apt install -y ros-humble-demo-nodes-cpp gz-garden \
    ros-humble-rqt-console ros-humble-pcl* libsuitesparse-dev \
    libboost-all-dev
